...
export async function GET(request: NextRequest) {
  const backendUrl = new URL(`${getBackendApiBaseUrl()}/leads`);
  ...
  try {
    ...
    return NextResponse.json(
      { ok: false, message: 'No se pudo conectar con backend para listar leads.' },
...
export async function POST(request: NextRequest) {
  ...
  try {
    const upstreamResponse = await fetch(`${getBackendApiBaseUrl()}/leads`, {
      method: 'POST',
      headers: {
    ...
    return NextResponse.json(
      { ok: false, message: 'No se pudo conectar con backend para crear lead.' },
      { status: 503, headers: { 'Cache-Control': 'no-store' } }
    );
  }
}
