...
export async function POST(
  ...
  if (!id?.trim()) {
    return NextResponse.json(
      { ok: false, message: 'Id de categoria invalido.' },
  ...
  try {
    const upstreamResponse = await fetch(`${getBackendApiBaseUrl()}/service-categories/${id}/images`, {
      method: 'POST',
      headers: {
    ...
    return NextResponse.json(
      { ok: false, message: 'No se pudo conectar con backend para registrar imagen.' },
