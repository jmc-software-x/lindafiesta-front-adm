...
export async function GET(request: NextRequest) {
  const backendUrl = new URL(`${getBackendApiBaseUrl()}/service-categories`);
  ...
  try {
    ...
    return NextResponse.json(
      { ok: false, message: 'No se pudo conectar con backend para listar categorias.' },
...
export async function POST(request: NextRequest) {
  ...
  try {
    const upstreamResponse = await fetch(`${getBackendApiBaseUrl()}/service-categories`, {
      method: 'POST',
      headers: {
    ...
    return NextResponse.json(
      { ok: false, message: 'No se pudo conectar con backend para crear categoria.' },
      { status: 503, headers: { 'Cache-Control': 'no-store' } }
    );
  }
}
